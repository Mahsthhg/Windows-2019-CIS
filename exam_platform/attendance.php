...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mb_strlen($name) < 3) {
        ...
        // بررسی تکراری بودن (IP یا نام)
        $s = $pdo->prepare("SELECT COUNT(*) FROM attendance_records WHERE attendance_id=? AND (ip_address=? OR student_name=?)");
        $s->execute([$att['id'], $ip, $name]);
        if ((int)$s->fetchColumn() > 0) {
            $error = 'شما قبلاً در این جلسه حضور خود را ثبت کرده‌اید';
        } else {
            $pdo->prepare("INSERT INTO attendance_records (attendance_id, student_name, national_code, class_number, ip_address) VALUES (?,?,?,?,?)")
                ->execute([$att['id'], $name, $national, $class, $ip]);
            $success = true;
        }
    }
}
