...
<!-- Reset attempt -->
<?php if (isset($_GET['reset']) && $selectedForm):
    $rid = validateInt($_GET['reset'], 1);
    if ($rid) {
        $s = $pdo->prepare("SELECT id FROM answers WHERE id=? AND form_id=?");
        $s->execute([$rid, $fid]);
        if ($s->fetch()) {
            $pdo->prepare("DELETE FROM answers WHERE id=?")->execute([$rid]);
            header('Location: results.php?form=' . $fid . '&msg=reset');
            exit();
        }
    }
endif; ?>
