...
if (isset($_GET['deactivate'])) {
    $aid = validateInt($_GET['deactivate'], 1);
        ...
        <div class="att-card">
            ...
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                ...
                <?php if ($att['is_active'] && $att['end_time'] && strtotime($att['end_time']) > $now): ?>
                    ...
                    <a href="?deactivate=<?= $att['id'] ?>" class="btn btn-warning btn-sm" onclick="return confirm('غیرفعال شود؟')">⛔</a>
                <?php else: ?>
                    <!-- فعال‌سازی -->
                ...
                <a href="?delete=<?= $att['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('حذف شود؟')">🗑️</a>
            </div>
        </div>
        <?php endforeach; ?>
        <?php else: ?>
