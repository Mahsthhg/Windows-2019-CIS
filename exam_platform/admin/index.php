...
// Recent teachers
$s = $pdo->query("SELECT u.*, COUNT(DISTINCT f.id) as exam_count, COUNT(DISTINCT k.id) as key_count FROM users u LEFT JOIN forms f ON f.teacher_id=u.id LEFT JOIN access_keys k ON k.teacher_id=u.id WHERE u.role='teacher' GROUP BY u.id ORDER BY u.id DESC LIMIT 10");
$teachers = $s->fetchAll();
...
// تغییر وضعیت/حذف معلم — فقط POST با CSRF (جلوگیری از حملهٔ CSRF روی لینک GET)
if (isset($_POST['toggle_teacher'])) {
    requireCsrf();
    $tid2 = validateInt($_POST['toggle_teacher'], 1);
    if ($tid2) $pdo->prepare("UPDATE users SET is_active=1-is_active WHERE id=? AND role='teacher'")->execute([$tid2]);
    redirect('index.php');
}
if (isset($_POST['delete_teacher'])) {
    requireCsrf();
    $tid2 = validateInt($_POST['delete_teacher'], 1);
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form method="POST" style="display:inline;">
                            <input type="hidden" name="csrf_token" value="<?= h($csrf) ?>">
                            <input type="hidden" name="toggle_teacher" value="<?= $t['id'] ?>">
                            <button type="submit" class="btn btn-secondary btn-sm"><?= $t['is_active'] ? '⛔' : '✅' ?></button>
                        </form>
                        <form method="POST" style="display:inline;" onsubmit="return confirm('حذف این معلم و همهٔ آزمون‌هایش؟')">
                            <input type="hidden" name="csrf_token" value="<?= h($csrf) ?>">
                            <input type="hidden" name="delete_teacher" value="<?= $t['id'] ?>">
